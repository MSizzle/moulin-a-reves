...
### Feedback mode (`?feedback=1` → auto-code → auto-deploy)
- **Sibling of the editor flow, NOT part of it.** The client opens the auth-gated `/feedback` page (`src/pages/feedback.astro`), which iframes the live site at `/?feedback=1&v=<VER>`. `BaseLayout.astro` has a SECOND inline loader block (right after the `?edit=1` one, which is left byte-for-byte unchanged) that injects `public/feedback-inject.js`. The click-to-edit editor flow (`?edit=1`, `editor-inject.js`, `editor/`, `site/save.ts`, `site/publish.ts`, `guardrails.js`) is untouched — keep feedback changes additive.
...
- **`feedback-incoming/` rule.** Client photo uploads are committed by `src/pages/api/feedback/submit.ts` to `feedback-incoming/issue-<n>/<file>` at the **repo root, NOT under `/public/`** (so `check-public-clean.mjs` never trips) and it is listed in `.vercelignore` (so it never deploys). The GitHub Action consumes the raw file via the Sharp pipeline onto the existing `/images/*.webp` target, then **deletes the raw `feedback-incoming/` path** in the same commit. Never serve or reference `feedback-incoming/` from site code.
- **The feedback pipeline self-codes.** A submission becomes a `client-feedback`-labelled issue whose body carries a fenced JSON locator + autonomy hint; `.github/workflows/claude.yml` runs `claude-code-action` under `.github/CLAUDE_FEEDBACK.md`. Read that manual before changing locator capture, the issue schema, or the autonomy gate — the client-side (`feedback-inject.js`), server-side (`submit.ts`), and Action (`CLAUDE_FEEDBACK.md`) validation rules are intentionally mirrored and must stay in sync.
